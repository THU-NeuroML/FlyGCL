...
This repository collects FlyGCL implementations across four application scenarios. Each scenario is self-contained because its data pipeline, backbone, dependencies, and evaluation protocol are substantially different.
...
The four directories are independent experiment packages. Create a separate environment for each one; there is intentionally no repository-wide `requirements.txt`.
...
Each guide describes the data paths and pretrained-model arguments already supported by its implementation.
