...
class VisionClassifier(nn.Module):
    def __init__(self, in_features, num_classes, weight_init=None, activation=None, beta_initial=4.0):
        super().__init__()
        ...
        self.beta = nn.Parameter(torch.tensor(beta_initial, dtype=torch.float32))
...
def load_model(cfg: DictConfig, device: torch.device) -> nn.Module:
    ...
    if cfg.scenario == "class":
        method = getattr(cfg, "method", "lora")
        if method == "lora":
            return ClassIncrementalCLIP(cfg, device)
        from .methods import METHOD_REGISTRY
        if method not in METHOD_REGISTRY:
            raise ValueError(f"Unknown method '{method}'. Choose from {list(METHOD_REGISTRY.keys())}")
        return METHOD_REGISTRY[method](cfg, device)
    else:
        raise ValueError(f"""
