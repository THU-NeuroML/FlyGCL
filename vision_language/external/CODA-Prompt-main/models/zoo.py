...
class CodaPrompt(nn.Module):
    def __init__(self, emb_d, n_tasks, prompt_param, key_dim=768, num_layers=None):
        super().__init__()
        self.task_count = 0
        self.emb_d = emb_d
        self.key_d = key_dim
        self.n_tasks = n_tasks
        self._init_smart(emb_d, prompt_param, num_layers=num_layers)
    ...
    def _init_smart(self, emb_d, prompt_param, num_layers=None):

        # prompt basic param
        self.e_pool_size = int(prompt_param[0])
        self.e_p_length = int(prompt_param[1])
        self.e_layers = list(range(int(num_layers))) if num_layers is not None else [0,1,2,3,4]
...
class DualPrompt(nn.Module):
    ...
    def _init_smart(self, emb_d, prompt_param, num_layers=None):
        ...
        if num_layers is None:
            ...
            self.g_layers = list(range(int(num_layers)))
            self.e_layers = list(range(int(num_layers)))
...
class L2P(DualPrompt):
    def __init__(self, emb_d, n_tasks, prompt_param, key_dim=768, num_layers=None):
        super().__init__(emb_d, n_tasks, prompt_param, key_dim, num_layers=num_layers)
