...
exec bash "${REPO_ROOT}/bash/dit_dec_run_seed.sh" \
  --seed="${SEED:-42}" \
  ...
  --checkpoint-root="${CHECKPOINT_ROOT:-./outputs/dit_dec/libero_goal}" \
  --source-checkpoint-root="${SOURCE_CHECKPOINT_ROOT:-${CHECKPOINT_ROOT:-./outputs/dit_dec/libero_goal}}" \
  --log-root="${LOG_ROOT:-./logs/dit_dec/libero_goal}" \
