...
BATCH_SIZE="${BATCH_SIZE:-16}"; REPLAY_BATCH_SIZE="${REPLAY_BATCH_SIZE:-16}"; NUM_WORKERS="${NUM_WORKERS:-16}"; REPLAY_NUM_WORKERS="${REPLAY_NUM_WORKERS:-16}"; STEPS="${STEPS:-20000}"; LOG_STEPS="${LOG_STEPS:-100}"; N_EVAL="${N_EVAL:-100}"; BS_EVAL="${BS_EVAL:-50}"; EVAL_MAX_EPISODES_RENDERED="${EVAL_MAX_EPISODES_RENDERED:-100}"; EVAL_FREQ="${EVAL_FREQ:-$STEPS}"
...
update_manifest_after_task() { local task="$1" previous_manifest="" output_manifest="" new_repo_id=""; output_manifest="$(manifest_after_task "$task")"; new_repo_id="${DATASET_PREFIX}_${task}"; cmd=("$PYTHON_BIN" "$MEMORY_SCRIPT_PATH" update "--output-manifest=${output_manifest}" "--new-repo-id=${new_repo_id}" "--capacity=${REPLAY_MEMORY_CAPACITY}" "--seed=${SEED}"); if (( task == 0 )); then cmd+=("--allow-missing-previous"); else previous_manifest="$(manifest_after_task "$((task - 1))")"; [[ -f "$previous_manifest" ]] || { echo "ERROR: previous reservoir manifest not found for task ${task}: ${previous_manifest}" >&2; exit 9; }; cmd+=("--previous-manifest=${previous_manifest}"); fi; cmd_string=""; printf -v cmd_string '%q ' "${cmd[@]}"; [[ "$SHOW_COMMAND" == "true" ]] && log_task_info "Manifest command: ${cmd_string}"; run_with_clean_stdout_logs "$cmd_string"; local cmd_exit_code=$?; (( cmd_exit_code == 0 )) || exit "$cmd_exit_code"; }
...
log_info "Repo root: $REPO_ROOT"; log_info "Launcher log: $MASTER_LOG"; log_info "Seed: $SEED"; log_info "Task range: ${TASK_START}-${TASK_END}"; log_info "ER reservoir: capacity=${REPLAY_MEMORY_CAPACITY}, current=${BATCH_SIZE}, replay=${REPLAY_BATCH_SIZE}"; log_info "Memory dir: $MEMORY_DIR"; log_info "Python: $PYTHON_BIN"
