...
usage() {
  cat <<'EOF'
Usage:
  bash/er_run_seed.sh [options] [-- extra er.py args...]

Runs ER for a contiguous task range. Task 0 is bootstrapped with SeqFFT because no previous replay data exists.
ER defaults to 50/50 current/replay batches for task >= 1: --batch_size=16 --replay_batch_size=16.
EOF
}

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"; cd "$REPO_ROOT"
ER_SCRIPT_PATH="./lerobot_lsy/src/lerobot/scripts/er.py"
...
JOB_PREFIX="${JOB_PREFIX:-er}"; JOB_SUFFIX="${JOB_SUFFIX:-reproduce}"; SOURCE_JOB_SUFFIX="${SOURCE_JOB_SUFFIX:-}"
CHECKPOINT_ROOT="${CHECKPOINT_ROOT:-./outputs}"; SOURCE_CHECKPOINT_ROOT="${SOURCE_CHECKPOINT_ROOT:-}"; LOG_ROOT="${LOG_ROOT:-./logs/er}"
POLICY_PATH="${POLICY_PATH:-./models/dit_flow_mt_libero_90_pretrain}"; INITIAL_POLICY_PATH="${INITIAL_POLICY_PATH:-}"
BENCHMARK_NAME="${BENCHMARK_NAME:-libero_10}"; TASK_PREFIX="${TASK_PREFIX:-Libero_10_Task}"; DATASET_PREFIX="${DATASET_PREFIX:-continuallearning/libero_10_image_task}"; REPLAY_DATASET_PREFIX="${REPLAY_DATASET_PREFIX:-$DATASET_PREFIX}"; REPLAY_DATASET_REPO_ID="${REPLAY_DATASET_REPO_ID:-}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --replay-dataset-prefix) REPLAY_DATASET_PREFIX="$2"; shift 2 ;; --replay-dataset-prefix=*) REPLAY_DATASET_PREFIX="${1#*=}"; shift ;;
    --replay-dataset-repo-id) REPLAY_DATASET_REPO_ID="$2"; shift 2 ;; --replay-dataset-repo-id=*) REPLAY_DATASET_REPO_ID="${1#*=}"; shift ;;
...
build_replay_repo_arg() { local task="$1"; if [[ -n "$REPLAY_DATASET_REPO_ID" ]]; then printf "%s" "$REPLAY_DATASET_REPO_ID"; return 0; fi; local joined="" i=""; for ((i = 0; i < task; i++)); do [[ -n "$joined" ]] && joined+=","; joined+="${REPLAY_DATASET_PREFIX}_${i}"; done; printf "%s" "$joined"; }
...
log_info "Repo root: $REPO_ROOT"; log_info "Launcher log: $MASTER_LOG"; log_info "Seed: $SEED"; log_info "Task range: ${TASK_START}-${TASK_END}"; log_info "ER batches: current=${BATCH_SIZE}, replay=${REPLAY_BATCH_SIZE}"; log_info "Python: $PYTHON_BIN"
for ((task = TASK_START; task <= TASK_END; task++)); do
  job_name="$(build_job_name "$SEED" "$task" "$JOB_SUFFIX")"; output_dir="$(build_output_dir "$CHECKPOINT_ROOT" "$SEED" "$task" "$JOB_SUFFIX")"; env_tasks="$(build_env_task_list "$task")"; current_policy_path="$(resolve_policy_path "$task")"; replay_repo_arg="$(build_replay_repo_arg "$task")"; task_log="${RUN_LOG_ROOT}/task_${task}.log"; task_err_log="${RUN_LOG_ROOT}/task_${task}.stderr.log"
  ...
  if (( task == 0 )); then
    ...
    cmd=("$PYTHON_BIN" "$ER_SCRIPT_PATH" "--seed=${SEED}" "--job_name=${job_name}" "--output_dir=${output_dir}" "--dataset.repo_id=${DATASET_PREFIX}_${task}" "--replay_dataset.repo_id=${REPLAY_DATASET_PREFIX}_0" "--replay_dataset_repo_ids=${replay_repo_arg}" "--policy.path=${current_policy_path}" "--policy.push_to_hub=false" "--batch_size=${BATCH_SIZE}" "--replay_batch_size=${REPLAY_BATCH_SIZE}" "--num_workers=${NUM_WORKERS}" "--replay_num_workers=${REPLAY_NUM_WORKERS}" "--steps=${STEPS}" "--env.type=libero" "--env.benchmark=${BENCHMARK_NAME}" "--env.task=${env_tasks}" "--eval.batch_size=${BS_EVAL}" "--eval.n_episodes=${N_EVAL}" "--eval.max_episodes_rendered=${EVAL_MAX_EPISODES_RENDERED}" "--eval_freq=${EVAL_FREQ}" "--save_freq=${STEPS}" "--log_freq=${LOG_STEPS}" "--wandb.enable=${WANDB_ENABLE}" "--wandb.disable_artifact=true" "--wandb.project=${WANDB_PROJECT}" "--wandb.entity=${WANDB_ENTITY}")
  fi
  [[ ${#EXTRA_ARGS[@]} -gt 0 ]] && cmd+=("${EXTRA_ARGS[@]}")
  log_task_info "==== Running seed ${SEED}, task ${task} ===="; log_task_info "Output dir: $output_dir"; log_task_info "Policy source: $current_policy_path"; if (( task > 0 )); then log_task_info "Replay repos: $replay_repo_arg"; fi; log_task_info "Task log: $task_log"; log_task_info "Task stderr log: $task_err_log"
