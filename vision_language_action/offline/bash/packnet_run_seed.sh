...
usage() {
  cat <<'EOF'
Usage:
  bash/packnet_run_seed.sh [options] [-- extra packnet.py args...]

Options mirror seqfft_run_seed.sh. PackNet defaults: --steps=20000 --post-prune-steps=20000 --prune-ratio=0.75.
EOF
}
...
JOB_PREFIX="${JOB_PREFIX:-packnet}"; JOB_SUFFIX="${JOB_SUFFIX:-reproduce}"; SOURCE_JOB_SUFFIX="${SOURCE_JOB_SUFFIX:-}"
CHECKPOINT_ROOT="${CHECKPOINT_ROOT:-./outputs}"; SOURCE_CHECKPOINT_ROOT="${SOURCE_CHECKPOINT_ROOT:-}"; LOG_ROOT="${LOG_ROOT:-./logs/packnet}"
...
PRUNE_RATIO="${PRUNE_RATIO:-0.75}"; POST_PRUNE_STEPS="${POST_PRUNE_STEPS:-20000}"; IGNORE_MODULES="${IGNORE_MODULES:-}"
SHOW_COMMAND=false
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --prune-ratio) PRUNE_RATIO="$2"; shift 2 ;; --prune-ratio=*) PRUNE_RATIO="${1#*=}"; shift ;;
    --post-prune-steps) POST_PRUNE_STEPS="$2"; shift 2 ;; --post-prune-steps=*) POST_PRUNE_STEPS="${1#*=}"; shift ;;
    --ignore-modules) IGNORE_MODULES="$2"; shift 2 ;; --ignore-modules=*) IGNORE_MODULES="${1#*=}"; shift ;;
...
METHOD_SLUG="prune${PRUNE_RATIO//./_}_post${POST_PRUNE_STEPS}"
...
log_info "Repo root: $REPO_ROOT"; log_info "Launcher log: $MASTER_LOG"; log_info "Seed: $SEED"; log_info "Task range: ${TASK_START}-${TASK_END}"; log_info "PackNet: prune_ratio=${PRUNE_RATIO}, post_prune_steps=${POST_PRUNE_STEPS}"; log_info "Python: $PYTHON_BIN"
for ((task = TASK_START; task <= TASK_END; task++)); do
  ...
  if (( task > 0 )) && [[ ! -f "${current_policy_path}/mask.safetensors" ]]; then echo "ERROR: PackNet mask not found for task ${task}: ${current_policy_path}/mask.safetensors" >&2; exit 7; fi
  mkdir -p "$(dirname "$output_dir")"
  cmd=("$PYTHON_BIN" "$SCRIPT_PATH" "--seed=${SEED}" "--job_name=${job_name}" "--output_dir=${output_dir}" "--dataset.repo_id=${DATASET_PREFIX}_${task}" "--policy.path=${current_policy_path}" "--policy.push_to_hub=false" "--batch_size=${BATCH_SIZE}" "--num_workers=${NUM_WORKERS}" "--steps=${STEPS}" "--post_prune_steps=${POST_PRUNE_STEPS}" "--current_task=${task}" "--prune_ratio=${PRUNE_RATIO}" "--ignore_modules=${IGNORE_MODULES}" "--env.type=libero" "--env.benchmark=${BENCHMARK_NAME}" "--env.task=${env_tasks}" "--eval.batch_size=${BS_EVAL}" "--eval.n_episodes=${N_EVAL}" "--eval.max_episodes_rendered=${EVAL_MAX_EPISODES_RENDERED}" "--eval_freq=${EVAL_FREQ}" "--save_freq=${STEPS}" "--log_freq=${LOG_STEPS}" "--wandb.enable=${WANDB_ENABLE}" "--wandb.disable_artifact=true" "--wandb.project=${WANDB_PROJECT}" "--wandb.entity=${WANDB_ENTITY}")
  [[ ${#EXTRA_ARGS[@]} -gt 0 ]] && cmd+=("${EXTRA_ARGS[@]}")
  log_task_info "==== Running seed ${SEED}, task ${task} ===="; log_task_info "Output dir: $output_dir"; log_task_info "Policy/mask source: $current_policy_path"; log_task_info "Task log: $task_log"; log_task_info "Task stderr log: $task_err_log"
