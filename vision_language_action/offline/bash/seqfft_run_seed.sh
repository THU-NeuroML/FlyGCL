...
usage() {
  cat <<'EOF'
Usage:
  bash/seqfft_run_seed.sh [options] [-- extra train_eval_multi.py args...]
...
Options:
  ...
  --job-prefix STR               Job name prefix. Default: seqfft
  --job-suffix STR               Job name suffix appended after method slug.
  --source-job-suffix STR        Suffix used to locate predecessor checkpoint for first task.
  --checkpoint-root PATH         Root directory for outputs. Default: ./outputs
  --source-checkpoint-root PATH  Root directory used to locate predecessor checkpoint.
  --log-root PATH                Root directory for launcher logs. Default: ./logs/seqfft
  --policy-path PATH             Base pretrained policy path.
  --initial-policy-path PATH     Explicit policy path for the first task in the range.
  --benchmark-name STR           LIBERO benchmark passed to --env.benchmark. Default: libero_10
  --task-prefix STR              Task name prefix before numeric task id. Default: Libero_10_Task
  --dataset-prefix STR           Dataset repo prefix before numeric task id. Default: continuallearning/libero_10_image_task
  --max-task INT                 Maximum allowed task index. Default: 9
  --env-task-override CSV        Explicit comma-separated task list forwarded to --env.task.
  --wandb-enable BOOL            true or false. Default: false
  --wandb-project STR            Default: baseline_experiments
  --wandb-entity STR             Default: 
  --show-command                 Print full python command before each task.
  --help                         Show this message.

Any arguments after '--' are forwarded to train_eval_multi.py unchanged.
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --seed) SEED="$2"; shift 2 ;;
    --seed=*) SEED="${1#*=}"; shift ;;
    --task) TASK_START="$2"; TASK_END="$2"; shift 2 ;;
    --task=*) TASK_START="${1#*=}"; TASK_END="${1#*=}"; shift ;;
    ...
    --source-checkpoint-root) SOURCE_CHECKPOINT_ROOT="$2"; shift 2 ;;
    --source-checkpoint-root=*) SOURCE_CHECKPOINT_ROOT="${1#*=}"; shift ;;
    --log-root) LOG_ROOT="$2"; shift 2 ;;
    --log-root=*) LOG_ROOT="${1#*=}"; shift ;;
    --policy-path) POLICY_PATH="$2"; shift 2 ;;
    --policy-path=*) POLICY_PATH="${1#*=}"; shift ;;
    --initial-policy-path) INITIAL_POLICY_PATH="$2"; shift 2 ;;
    --initial-policy-path=*) INITIAL_POLICY_PATH="${1#*=}"; shift ;;
    ...
    --max-task) MAX_TASK="$2"; shift 2 ;;
    --max-task=*) MAX_TASK="${1#*=}"; shift ;;
    --env-task-override) ENV_TASK_OVERRIDE="$2"; shift 2 ;;
    --env-task-override=*) ENV_TASK_OVERRIDE="${1#*=}"; shift ;;
...
[[ -z "$SOURCE_CHECKPOINT_ROOT" ]] && SOURCE_CHECKPOINT_ROOT="$CHECKPOINT_ROOT"

if [[ ! "$SEED" =~ ^[0-9]+$ || ! "$TASK_START" =~ ^[0-9]+$ || ! "$TASK_END" =~ ^[0-9]+$ || ! "$MAX_TASK" =~ ^[0-9]+$ ]]; then
  echo "ERROR: seed and task indices must be non-negative integers." >&2
  exit 2
fi
...
if [[ -z "$PYTHON_BIN" || ! -x "$PYTHON_BIN" ]]; then echo "ERROR: could not resolve a runnable python interpreter." >&2; exit 8; fi
...
run_with_clean_stdout_logs() {
  local cmd_string="$1"
  set +e
  bash -lc "$cmd_string" > >(tee -a "$MASTER_LOG" "$task_log") 2> >(tee -a "$MASTER_ERR_LOG" "$task_err_log" >&2)
  local cmd_exit_code=$?
  set -e
...
build_job_name() {
  local seed="$1" task="$2" suffix="$3" suffix_part=""
  [[ -n "$suffix" ]] && suffix_part="_${suffix}"
  printf "%s_seed_%s_%s_task_%s_%s%s" "$JOB_PREFIX" "$seed" "$BENCHMARK_NAME" "$task" "$METHOD_SLUG" "$suffix_part"
}
build_output_dir() { printf "%s/%s" "$1" "$(build_job_name "$2" "$3" "$4")"; }
build_env_task_list() {
  local end_task="$1"
  if [[ -n "$ENV_TASK_OVERRIDE" ]]; then printf "%s" "$ENV_TASK_OVERRIDE"; return 0; fi
  local joined="" i=""
  for ((i = 0; i <= end_task; i++)); do
    [[ -n "$joined" ]] && joined+=","
    joined+="${TASK_PREFIX}_${i}"
  done
  printf "%s" "$joined"
}
resolve_policy_path() {
  local task="$1"
  if (( task == TASK_START )) && [[ -n "$INITIAL_POLICY_PATH" ]]; then printf "%s" "$INITIAL_POLICY_PATH"; return 0; fi
  if (( task == 0 )); then printf "%s" "$POLICY_PATH"; return 0; fi
  if (( task == TASK_START )); then printf "%s/checkpoints/last/pretrained_model" "$(build_output_dir "$SOURCE_CHECKPOINT_ROOT" "$SEED" "$((task - 1))" "$SOURCE_JOB_SUFFIX")"; return 0; fi
  printf "%s/checkpoints/last/pretrained_model" "$(build_output_dir "$CHECKPOINT_ROOT" "$SEED" "$((task - 1))" "$JOB_SUFFIX")"
}
...
MASTER_ERR_LOG="${RUN_LOG_ROOT}/seed_${SEED}_tasks_${TASK_START}_${TASK_END}.stderr.log"

log_info "Repo root: $REPO_ROOT"
log_info "Launcher log: $MASTER_LOG"
log_info "Launcher stderr log: $MASTER_ERR_LOG"
log_info "Seed: $SEED"
...
for ((task = TASK_START; task <= TASK_END; task++)); do
  ...
  current_policy_path="$(resolve_policy_path "$task")"
  task_log="${RUN_LOG_ROOT}/task_${task}.log"
  task_err_log="${RUN_LOG_ROOT}/task_${task}.stderr.log"

  if [[ ! -d "$current_policy_path" ]]; then
    echo "ERROR: policy checkpoint not found for task ${task}: $current_policy_path" >&2
    exit 7
  fi
  ...
  cmd=(
    ...
    "--policy.path=${current_policy_path}"
  ...
  log_task_info "Policy source: $current_policy_path"
  log_task_info "Task log: $task_log"
  log_task_info "Task stderr log: $task_err_log"
  cmd_string=""
  printf -v cmd_string '%q ' "${cmd[@]}"
  [[ "$SHOW_COMMAND" == "true" ]] && log_task_info "Command: ${cmd_string}"
  run_with_clean_stdout_logs "$cmd_string"
  cmd_exit_code=$?
  if (( cmd_exit_code != 0 )); then exit "$cmd_exit_code"; fi
done
