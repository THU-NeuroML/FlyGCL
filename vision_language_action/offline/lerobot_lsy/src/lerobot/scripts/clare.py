...
def train(cfg: PEFTTrainPipelineConfig):
    ...
    for _ in range(init_step, init_step + total_steps):
        start_time = time.perf_counter()
        batch = next(dl_iter)
        ...
        if discriminator_optimizer is not None and step == init_step + cfg.steps:
        ...
        if step <= init_step + cfg.steps:
            ...
            is_saving_step = (step - init_step) % save_freq == 0 or step == init_step + total_steps
