...
class ERTrainPipelineConfig(TrainPipelineConfig):
    ...
    replay_dataset_repo_ids: str | None = None
...
def make_replay_dataset(cfg: ERTrainPipelineConfig) -> LeRobotDataset | torch.utils.data.ConcatDataset:
    ...
    if cfg.replay_dataset is None:
        raise ValueError("replay_dataset must be provided for ER training.")
    ...
    repo_ids = None
    if cfg.replay_dataset_repo_ids:
        repo_ids = [repo_id.strip() for repo_id in cfg.replay_dataset_repo_ids.split(",") if repo_id.strip()]
    elif isinstance(cfg.replay_dataset.repo_id, str):
        repo_ids = [cfg.replay_dataset.repo_id]
    else:
        repo_ids = list(cfg.replay_dataset.repo_id)

    if not repo_ids:
        raise ValueError("At least one replay repo id must be provided.")

    if len(repo_ids) == 1:
        ds_meta = LeRobotDatasetMetadata(
            repo_ids[0], root=cfg.replay_dataset.root, revision=cfg.replay_dataset.revision
        )
        ...
        meta = LeRobotDatasetMetadata(repo_ids[0], root=cfg.replay_dataset.root, revision=cfg.replay_dataset.revision)
        ...
        logging.info(f"Created replay dataset from {len(repo_ids)} repos: {repo_ids}")
