...
class PeftConfigMixin(PushToHubMixin):
    ...
    def save_pretrained(self, save_directory: str, **kwargs) -> None:
        ...
        if "_module_configs" in output_dict:
            for pattern in output_dict["_module_configs"]:
                if "discriminator_cfg" in output_dict["_module_configs"][pattern] and self._module_configs[pattern].discriminator_cfg is not None:
                    ...
                    elif isinstance(output_dict["target_modules"], list):
                        for module_cfg in output_dict["target_modules"]:
                            if module_cfg.get("pattern") == pattern and module_cfg.get("discriminator_cfg") is not None:
                                module_cfg["discriminator_cfg"]["type"] = self._module_configs[pattern].discriminator_cfg.type
    ...
    def from_peft_type(cls, **kwargs):
        ...
        kwargs = config_cls.check_kwargs(**kwargs)
