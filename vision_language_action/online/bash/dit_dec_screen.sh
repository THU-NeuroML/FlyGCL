...
usage() {
  cat <<'EOF'
Usage:
  bash/dit_dec_screen.sh <start|status|attach|stop> [options] [-- extra launcher args...]
...
Options:
  ...
  --session-prefix STR           Screen session prefix. Default: dit-dec
  --seed INT                     Required by attach to select one session.
  --help                         Show this message.

Any arguments after '--' are forwarded to bash/dit_dec_run_seed.sh.
EOF
}
...
CHECKPOINT_BASE="${CHECKPOINT_BASE:-./outputs/dit_dec}"
...
SESSION_PREFIX="${SESSION_PREFIX:-dit-dec}"
...
start_sessions() {
  ...
  for idx in "${!SEEDS[@]}"; do
    ...
    local cmd=(
      bash
      "${REPO_ROOT}/bash/dit_dec_run_seed.sh"
      "--seed=${seed}"
      "--gpu-id=${gpu}"
