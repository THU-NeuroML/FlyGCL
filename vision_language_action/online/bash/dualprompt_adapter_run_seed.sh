...
usage() {
  cat <<'EOF'
Usage:
  bash/dualprompt_adapter_run_seed.sh [options] [-- extra dualprompt_adapter.py args...]

Runs DualPrompt-style general+expert adapter continual learning for a contiguous task range.
EOF
}
...
TASK_END="${TASK_END:-9}"
GPU_ID="${GPU_ID:-}"
JOB_PREFIX="${JOB_PREFIX:-dualprompt_adapter}"
...
LOG_ROOT="${LOG_ROOT:-./logs/dualprompt_adapter}"
...
METHOD_SLUG="general1_expert1_proto200"
log_info() { printf '%s\n' "$*" | tee -a "$MASTER_LOG"; }
...
resolve_peft_source() { local task="$1"; if (( task == 0 )); then printf "%s" "$PEFT_CFG_PATH"; return 0; fi; if (( task == TASK_START )); then printf "%s/checkpoints/last/adapter" "$(build_output_dir "$SOURCE_CHECKPOINT_ROOT" "$SEED" "$((task - 1))" "$SOURCE_JOB_SUFFIX")"; return 0; fi; printf "%s/checkpoints/last/adapter" "$(build_output_dir "$CHECKPOINT_ROOT" "$SEED" "$((task - 1))" "$JOB_SUFFIX")"; }
resolve_policy_path() { local task="$1"; if (( task == TASK_START )) && [[ -n "$INITIAL_POLICY_PATH" ]]; then printf "%s" "$INITIAL_POLICY_PATH"; return 0; fi; printf "%s" "$POLICY_PATH"; }
...
log_info "PEFT config: $PEFT_CFG_PATH"
...
for ((task = TASK_START; task <= TASK_END; task++)); do
  ...
  peft_source="$(resolve_peft_source "$task")"
  ...
  [[ -e "$peft_source" ]] || { echo "ERROR: PEFT source not found for task ${task}: $peft_source" >&2; exit 9; }
  ...
  if (( task == 0 )); then cmd+=("--peft_cfg_path=${peft_source}"); else cmd+=("--peft_weight_path=${peft_source}"); fi
  [[ ${#EXTRA_ARGS[@]} -gt 0 ]] && cmd+=("${EXTRA_ARGS[@]}")

  log_task_info "==== Running DualPrompt adapter seed ${SEED}, task ${task} ===="
  log_task_info "Output dir: $output_dir"
  log_task_info "Policy source: $current_policy_path"
  log_task_info "PEFT source: $peft_source"
...
log_info "Completed DualPrompt adapter seed ${SEED}, tasks ${TASK_START}-${TASK_END}."
