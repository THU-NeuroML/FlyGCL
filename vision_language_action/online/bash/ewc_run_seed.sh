...
usage() {
  cat <<'EOF'
Usage:
  bash/ewc_run_seed.sh [options] [-- extra ewc.py args...]

Runs EWC for a contiguous task range. Task 0 has no EWC penalty; task >= 1 loads the previous task's cumulative EWC state.
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --ewc-lambda) EWC_LAMBDA="$2"; shift 2 ;; --ewc-lambda=*) EWC_LAMBDA="${1#*=}"; shift ;;
    --ewc-fisher-batches) EWC_FISHER_BATCHES="$2"; shift 2 ;; --ewc-fisher-batches=*) EWC_FISHER_BATCHES="${1#*=}"; shift ;;
    --ewc-fisher-batch-size) EWC_FISHER_BATCH_SIZE="$2"; shift 2 ;; --ewc-fisher-batch-size=*) EWC_FISHER_BATCH_SIZE="${1#*=}"; shift ;;
...
[[ -n "$PYTHON_BIN" && -x "$PYTHON_BIN" ]] || { echo "ERROR: could not resolve a runnable python interpreter." >&2; exit 8; }

METHOD_SLUG="ewc_l1000_fb200"
log_info() { printf '%s\n' "$*" | tee -a "$MASTER_LOG"; }
...
resolve_ewc_state_path() { local task="$1"; if (( task == 0 )); then printf ""; return 0; fi; if (( task == TASK_START )); then printf "%s/ewc_state.pt" "$(build_output_dir "$SOURCE_CHECKPOINT_ROOT" "$SEED" "$((task - 1))" "$SOURCE_JOB_SUFFIX")"; return 0; fi; printf "%s/ewc_state.pt" "$(build_output_dir "$CHECKPOINT_ROOT" "$SEED" "$((task - 1))" "$JOB_SUFFIX")"; }
...
log_info "EWC lambda: $EWC_LAMBDA"
log_info "EWC Fisher batches: $EWC_FISHER_BATCHES"
...
for ((task = TASK_START; task <= TASK_END; task++)); do
  ...
  ewc_state_path="$(resolve_ewc_state_path "$task")"
...
log_info "Completed EWC seed ${SEED}, tasks ${TASK_START}-${TASK_END}."
