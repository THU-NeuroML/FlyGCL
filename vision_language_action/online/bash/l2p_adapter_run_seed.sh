...
usage() {
  cat <<'EOF'
Usage:
  bash/l2p_adapter_run_seed.sh [options] [-- extra l2p_adapter.py args...]

Runs L2P-style adapter-pool continual learning for a contiguous task range.
EOF
}
...
TASK_END="${TASK_END:-9}"
GPU_ID="${GPU_ID:-}"
JOB_PREFIX="${JOB_PREFIX:-l2p_adapter}"
...
LOG_ROOT="${LOG_ROOT:-./logs/l2p_adapter}"
...
METHOD_SLUG="pool10_top1_key0_1"
log_info() { printf '%s\n' "$*" | tee -a "$MASTER_LOG"; }
...
for ((task = TASK_START; task <= TASK_END; task++)); do
  ...
  log_task_info "==== Running L2P adapter seed ${SEED}, task ${task} ===="
...
log_info "Completed L2P adapter seed ${SEED}, tasks ${TASK_START}-${TASK_END}."
