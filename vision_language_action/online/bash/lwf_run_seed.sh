...
usage() {
  cat <<'EOF'
Usage:
  bash/lwf_run_seed.sh [options] [-- extra lwf.py args...]

Runs LwF for a contiguous task range. Task 0 has no teacher; task >= 1 uses the previous task checkpoint as the frozen teacher.
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --lwf-lambda) LWF_LAMBDA="$2"; shift 2 ;; --lwf-lambda=*) LWF_LAMBDA="${1#*=}"; shift ;;
...
resolve_teacher_policy_path() { local task="$1"; if (( task == 0 )); then printf ""; return 0; fi; if (( task == TASK_START )); then printf "%s/checkpoints/last/pretrained_model" "$(build_output_dir "$SOURCE_CHECKPOINT_ROOT" "$SEED" "$((task - 1))" "$SOURCE_JOB_SUFFIX")"; return 0; fi; printf "%s/checkpoints/last/pretrained_model" "$(build_output_dir "$CHECKPOINT_ROOT" "$SEED" "$((task - 1))" "$JOB_SUFFIX")"; }
...
log_info "LwF lambda: $LWF_LAMBDA"
...
for ((task = TASK_START; task <= TASK_END; task++)); do
  ...
  teacher_policy_path="$(resolve_teacher_policy_path "$task")"
  ...
  if (( task > 0 )); then [[ -d "$teacher_policy_path" ]] || { echo "ERROR: teacher checkpoint not found for task ${task}: $teacher_policy_path" >&2; exit 9; }; fi
  ...
  (( task > 0 )) && cmd+=("--teacher_policy_path=${teacher_policy_path}")
  [[ ${#EXTRA_ARGS[@]} -gt 0 ]] && cmd+=("${EXTRA_ARGS[@]}")

  log_task_info "==== Running LwF seed ${SEED}, task ${task} ===="
  log_task_info "Output dir: $output_dir"
  log_task_info "Policy source: $current_policy_path"
  (( task > 0 )) && log_task_info "Teacher source: $teacher_policy_path"
...
log_info "Completed LwF seed ${SEED}, tasks ${TASK_START}-${TASK_END}."
