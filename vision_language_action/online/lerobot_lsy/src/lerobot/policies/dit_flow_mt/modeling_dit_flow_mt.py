...
from transformers import CLIPTextModel, CLIPTokenizer, AutoModel, Dinov2Config, Dinov2Model
...
class LanguageEncoder(nn.Module):
    ...
    def __init__(self, config:DiTFlowMTConfig):
        ...
        _clip_path = config.language_model_name
        self.tokenizer = CLIPTokenizer.from_pretrained(_clip_path)
        self.clip_model = CLIPTextModel.from_pretrained(_clip_path)
        self.cache = {}
...
class DINOv2Encoder(nn.Module):
    def __init__(self, config: DiTFlowMTConfig):
        ...
        try:
            ...
            if config.vit_name != "facebook/dinov2-base":
                raise
            print(
                "DINOv2Encoder: falling back to offline Dinov2Model(Dinov2Config()) "
                f"for {config.vit_name}; checkpoint weights must be loaded afterwards. "
                f"Original error: {exc}",
                flush=True,
            )
            self._model = Dinov2Model(Dinov2Config(image_size=518))
