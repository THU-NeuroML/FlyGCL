...
def resolve_metadata_root(repo_id: str, root: str | None) -> str | None:
    if root is None:
        return None
    root_path = Path(root)
    candidates = [
        root_path / repo_id,
        root_path / repo_id.split(chr(47))[-1],
        root_path,
    ]
    for candidate in candidates:
        if (candidate / Path(chr(109)+chr(101)+chr(116)+chr(97)) / Path(chr(105)+chr(110)+chr(102)+chr(111)+chr(46)+chr(106)+chr(115)+chr(111)+chr(110))).is_file():
            return str(candidate)
    return str(root_path)


def list_repo_episodes(repo_id: str, root: str | None, revision: str | None) -> list[int]:
    meta_root = resolve_metadata_root(repo_id, root)
    meta = LeRobotDatasetMetadata(repo_id, root=meta_root, revision=revision)
