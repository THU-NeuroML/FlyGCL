...
@torch.no_grad()
def prune(
    cfg: PackNetTrainPipelineConfig,
    policy: torch.nn.Module,
    previous_mask: dict,
    current_task: int | None = None,
):
    """
    Prune cfg.prune_ratio of the active task's weights (by magnitude).
    mask keys are layer names (from named_modules).
    Returns new mask dict.
    """
    active_task = cfg.current_task if current_task is None else current_task
    current_masks = {}
    ...
    for name, module in policy.named_modules():
        ...
        # Select weights belonging to the active PackNet task/stage.
        select = layer_mask.eq(active_task + 1)
...
def assign_free_capacity_to_task(mask: dict, current_task: int):
    """Assign weights freed by pruning to the next active PackNet task/stage."""
    for layer_mask in mask.values():
        layer_mask[layer_mask.eq(0)] = current_task + 1
...
def train(cfg: PackNetTrainPipelineConfig):
    ...
    ignore_modules = [ignore_module.strip() for ignore_module in (cfg.ignore_modules or "").split(",") if ignore_module.strip()]
    ...
    active_packnet_task = cfg.current_task
    ...
    for _ in range(step, cfg.steps + cfg.post_prune_steps):
        ...
        current_gcl_stage, dl_iter = maybe_switch_gcl_stage(
            gcl_datasets, cfg, device, min(step, cfg.steps - 1), current_gcl_stage, dl_iter
        )
        if gcl_datasets is not None and current_gcl_stage is not None:
            if previous_gcl_stage is None:
                active_packnet_task = current_gcl_stage
            elif current_gcl_stage != previous_gcl_stage:
                logging.info(
                    "GCL PackNet: pruning stage %s before training stage %s",
                    previous_gcl_stage,
                    current_gcl_stage,
                )
                mask = prune(cfg, policy, mask, current_task=active_packnet_task)
                active_packnet_task = current_gcl_stage
                assign_free_capacity_to_task(mask, active_packnet_task)
                logging.info("GCL PackNet: assigned freed capacity to stage %s", active_packnet_task)
        start_time = time.perf_counter()
        batch = next(dl_iter)
        ...
        train_tracker, output_dict = update_policy(
            ...
            mask=mask,
            current_task=active_packnet_task,
        ...
        if cfg.env and is_eval_step:
            step_id = get_step_identifier(step, cfg.steps + cfg.post_prune_steps)
        ...
        if cfg.save_checkpoint and is_saving_step:
            ...
            checkpoint_dir = get_step_checkpoint_dir(cfg.output_dir, cfg.steps + cfg.post_prune_steps, step)
        ...
        if step == cfg.steps:
            logging.info("Prune the mask for active PackNet task/stage %s", active_packnet_task)
            mask = prune(cfg, policy, mask, current_task=active_packnet_task)
