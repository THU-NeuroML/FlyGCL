...
def make_merged_policy_for_eval_or_save(peft_policy: PeftModel) -> PreTrainedPolicy:
    peft_policy_copy = copy.deepcopy(peft_policy)
    if peft_policy_copy.peft_type == PeftType.LORA:
        peft_wrapper = peft_policy_copy.merge_and_unload()
    else:
        peft_wrapper = peft_policy_copy.unload()
    return peft_wrapper.policy
...
def train(cfg: PEFTTrainPipelineConfig):
    ...
    peft_modules = getattr(peft_policy.base_model, "adapter_layers", [])
    adapter_params = [p for p in peft_policy.parameters() if p.requires_grad]
    use_standard_peft_stage_expand = getattr(cfg.dataset, "use_gcl_expand", False) and not peft_modules
    if getattr(cfg.dataset, "use_gcl_expand", False) and peft_modules:
        for peft_module in peft_modules:
            ...
            if hasattr(peft_config, "structure"):
    ...
    num_learnable_params = sum(p.numel() for p in peft_policy.parameters() if p.requires_grad)
    num_total_params = sum(p.numel() for p in peft_policy.parameters())
    ...
    for _ in range(step, cfg.steps):
        ...
        if (
            ...
            and current_gcl_stage is not None
            and (previous_gcl_stage is not None or current_gcl_stage > 0)
            and current_gcl_stage != previous_gcl_stage
        ):
            logging.info(
                "GCL_EXPAND SeqLoRA: freezing stage %s adapter and creating stage %s adapter",
                previous_gcl_stage,
                current_gcl_stage,
            )
            adapter_params = []
            if use_standard_peft_stage_expand:
                adapter_name = "default" if current_gcl_stage == 0 else f"stage_{current_gcl_stage}"
                if adapter_name not in peft_policy.peft_config:
                    new_peft_config = copy.deepcopy(peft_config)
                    new_peft_config.inference_mode = False
                    peft_policy.add_adapter(adapter_name, new_peft_config)
                peft_policy.set_adapter(adapter_name)
                adapter_params = [p for p in peft_policy.parameters() if p.requires_grad]
            else:
                for peft_module in peft_modules:
                    ...
                    new_adapter_id = peft_module.num_adapters - 1
                    adapter_params.extend(peft_module.unfreeze_adapter(new_adapter_id))
                    key = f"{peft_module.layer_name}.{peft_module.layer_id}"
                    peft_module._forwarded_adapter_id = new_adapter_id
                    peft_module._forwarded_discriminator_id = -1
                    if hasattr(peft_config, "structure"):
                        peft_config.structure[key] = [peft_module.num_adapters, peft_module.num_discriminators]
            if not adapter_params:
                raise RuntimeError(
                    f"GCL_EXPAND could not find trainable adapter params for stage {current_gcl_stage}"
                )
            remaining_steps = max(cfg.steps - step, 1)
            optimizer = cfg.optimizer.build(adapter_params)
            lr_scheduler = cfg.scheduler.build(optimizer, remaining_steps) if cfg.scheduler is not None else None
            grad_scaler = GradScaler(device.type, enabled=cfg.policy.use_amp)
        ...
        if cfg.save_checkpoint and is_saving_step:
            ...
            policy_to_save = make_merged_policy_for_eval_or_save(peft_policy)
        ...
        if cfg.env and is_eval_step:
            ...
            for task in eval_envs.keys():
                ...
                with (
                    ...
                    eval_policy = make_merged_policy_for_eval_or_save(peft_policy)
