    ...
    **Attributes**:
    ...
    def save_pretrained(
        ...
        **kwargs: Any,
        ...
        for adapter_name in selected_adapters:
            ...
            if is_main_process and hasattr(self.base_model, "save_router_state_pretrained"):
                self.base_model.save_router_state_pretrained(
                    output_dir,
    ...
    def from_pretrained(
        ...
        **kwargs: Any,
        ...
        if hasattr(model.base_model, "load_router_state_pretrained"):
            router_state_path = model.base_model.load_router_state_pretrained(model_id)
            if router_state_path is not None:
                print(f"Loaded PEFT router state from {router_state_path}")
    ...
    def load_adapter(
        ...
        **kwargs: Any,
        ...
        if hasattr(self.base_model, "uses_router") and self.base_model.uses_router():
            router_state = None
            router_filenames = (
                self.base_model.router_checkpoint_filenames()
                if hasattr(self.base_model, "router_checkpoint_filenames")
                else (self.base_model.router_safe_weights_name, self.base_model.router_weights_name)
            )
            if os.path.isdir(model_id):
                for filename in router_filenames:
                    router_path = os.path.join(model_id, filename)
                    if not os.path.exists(router_path):
                        continue
                    if filename.endswith(".safetensors"):
                        router_state = safe_load_file(router_path)
                    else:
                ...
                for filename in router_filenames:
                    ...
                    if filename.endswith(".safetensors"):
                        router_state = safe_load_file(router_file)
                    else:
                        router_state = torch.load(router_file, map_location="cpu")
                    break

            if router_state is None:
                raise FileNotFoundError(
                    f"Missing router checkpoint for CLARE adapter at {model_id}. Expected one of "
                    f"{', '.join(router_filenames)}."
                )

            self.base_model.load_router_state_dict(router_state)
