...
class L2PKeyRouter(nn.Module):
    """Trainable cosine key router for adapterized L2P."""

    def __init__(self, feature_dim: int, pool_size: int, key_init: str = "normal") -> None:
        super().__init__()
        if feature_dim is None or feature_dim <= 0:
            raise ValueError(f"feature_dim must be positive, got {feature_dim}")
        if pool_size <= 0:
        ...
        self.adapter_keys = nn.Parameter(torch.empty(pool_size, feature_dim))
        self.reset_parameters(key_init)

    def reset_parameters(self, key_init: str) -> None:
        if key_init == "normal":
            nn.init.normal_(self.adapter_keys, mean=0.0, std=0.02)
        elif key_init == "uniform":
            nn.init.uniform_(self.adapter_keys, a=-0.02, b=0.02)
        else:
            raise ValueError(f"Unsupported l2p_key_init: {key_init}")
    ...
    def similarities(self, x: torch.Tensor, detach_query: bool = False) -> torch.Tensor:
        query = self.pool_features(x)
        if detach_query:
            query = query.detach()
    ...
    def forward(
        ...
        similarities = self.similarities(x, detach_query=detach_query)
        if active_pool_size is not None:
            active_pool_size = max(1, min(int(active_pool_size), similarities.shape[-1]))
            similarities = similarities[..., :active_pool_size]
        if top_k != 1:
            raise NotImplementedError("Adapterized L2P currently supports only top-1 routing.")
        scores, indices = torch.topk(similarities, k=1, dim=-1)
        return indices.squeeze(dim=-1), scores.squeeze(dim=-1)


class DualPromptPrototypeRouter(nn.Module):
    """Cosine prototype router for adapterized DualPrompt expert selection."""

    def __init__(self, feature_dim: int, max_tasks: int) -> None:
        super().__init__()
        if feature_dim is None or feature_dim <= 0:
            raise ValueError(f"feature_dim must be positive, got {feature_dim}")
        if max_tasks <= 0:
        ...
        self.register_buffer("task_prototypes", torch.zeros(max_tasks, feature_dim))
        self.register_buffer("expert_adapter_ids", torch.full((max_tasks,), -1, dtype=torch.long))
        self.register_buffer("valid_task_mask", torch.zeros(max_tasks, dtype=torch.bool))
        self.register_buffer("num_tasks", torch.tensor(0, dtype=torch.long))
        self.register_buffer("general_adapter_id", torch.tensor(-1, dtype=torch.long))
    ...
    @torch.no_grad()
    def set_general_adapter_id(self, adapter_id: int) -> None:
        self.general_adapter_id.fill_(int(adapter_id))

    @torch.no_grad()
    def set_task_prototype(self, task_id: int, expert_adapter_id: int, prototype: torch.Tensor) -> None:
        task_id = int(task_id)
        if task_id < 0 or task_id >= self.max_tasks:
            raise ValueError(f"task_id {task_id} is outside dual_max_tasks={self.max_tasks}")
        prototype = prototype.detach().to(device=self.task_prototypes.device, dtype=self.task_prototypes.dtype).reshape(-1)
        if prototype.numel() != self.feature_dim:
            raise ValueError(f"Prototype dim mismatch: expected {self.feature_dim}, got {prototype.numel()}")
        self.task_prototypes[task_id].copy_(prototype)
        self.expert_adapter_ids[task_id].fill_(int(expert_adapter_id))
        self.valid_task_mask[task_id].fill_(True)
        if task_id + 1 > int(self.num_tasks.item()):
            self.num_tasks.fill_(task_id + 1)

    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        features = self.pool_features(x)
        valid_indices = torch.nonzero(self.valid_task_mask, as_tuple=False).flatten()
        if valid_indices.numel() == 0:
            raise RuntimeError("DualPrompt prototype router has no valid task prototypes for evaluation.")
        query = F.normalize(features, dim=-1)
        prototypes = F.normalize(self.task_prototypes[valid_indices], dim=-1)
        similarities = query @ prototypes.T
        best_local = torch.argmax(similarities, dim=-1)
        task_ids = valid_indices[best_local]
        adapter_ids = self.expert_adapter_ids[task_ids]
        return adapter_ids, task_ids, similarities
...
class CLARELayer(nn.Module, BaseTunerLayer):
    def __init__(
        ...
        if self.routing_mode == "l2p_adapter" and self.routing_role != "router_follower":
            self.l2p_router = L2PKeyRouter(
                feature_dim=self.module_config.feature_dim,
                pool_size=self.peft_config.l2p_pool_size,
                key_init=self.peft_config.l2p_key_init,
            )
        if self.routing_mode == "dualprompt_adapter" and self.routing_role != "router_follower":
            self.dualprompt_router = DualPromptPrototypeRouter(
                feature_dim=self.module_config.feature_dim,
                max_tasks=self.peft_config.dual_max_tasks,
            )
    ...
    def _get_eval_adapter_outputs(
        ...
        **kwargs,
        ...
        if self.use_lora:
            ...
            if (
                self.routing_mode in {"l2p_adapter", "dualprompt_adapter"}
                and os.environ.get("PEFT_L2PDP_ONLINE_ONLY", "0") == "1"
            ):
                return outputs[0]
    ...
    def _forward_training_adapter_result(self, x: torch.Tensor) -> torch.Tensor:
        if self.routing_mode == "l2p_adapter":
            return self._forward_l2p_training_adapter_result(x)
        if self.routing_mode == "dualprompt_adapter":
            return self._forward_dualprompt_training_adapter_result(x)
        if not self.residual_ensemble_gate_enabled:
    ...
    def _forward_l2p_training_adapter_result(self, x: torch.Tensor) -> torch.Tensor:
        adapter_input = self._get_adapter_input(x)
        if self._forwarded_adapter_id >= 0:
            adapter_ids = [int(self._forwarded_adapter_id)] * adapter_input.shape[0]
            self._info_dicts["top_1_idx_list"] = adapter_ids
            if hasattr(self, "l2p_router"):
                similarities = self.l2p_router.similarities(adapter_input, detach_query=False)
                target_adapter_id = max(0, min(int(self._forwarded_adapter_id), similarities.shape[-1] - 1))
                self._info_dicts["l2p_scores"] = similarities[:, target_adapter_id]
                self._info_dicts["l2p_target_adapter_id"] = target_adapter_id
        else:
            adapter_ids = self._route_with_l2p_adapter(x)
    ...
    def _forward_dualprompt_training_adapter_result(self, x: torch.Tensor) -> torch.Tensor:
        general_adapter_id = self.get_dualprompt_general_adapter_id()
        expert_adapter_id = self._forwarded_adapter_id
        if general_adapter_id < 0 or expert_adapter_id < 0:
            raise RuntimeError("DualPrompt training requires both a general adapter id and a current expert adapter id.")
        general_result = self.clare_func_adapters[self.adapter_name][general_adapter_id](x)
        expert_result = self.clare_func_adapters[self.adapter_name][expert_adapter_id](x)
        return general_result + expert_result
    ...
    def _route_with_l2p_adapter(self, x: torch.Tensor) -> list[int]:
        if self.routing_role == "router_follower":
            adapter_ids = self._get_cached_routed_adapter_ids()
            self._info_dicts["top_1_idx_list"] = adapter_ids
            return adapter_ids
        if not hasattr(self, "l2p_router"):
            raise RuntimeError("L2P adapter routing is unavailable for router_follower modules.")
        adapter_input = self._get_adapter_input(x)
        routed_ids, routed_scores = self.l2p_router(
            adapter_input,
            top_k=self.peft_config.l2p_top_k,
            detach_query=True,
            active_pool_size=self.num_adapters,
        )
        adapter_ids = routed_ids.tolist()
        self._info_dicts["l2p_scores"] = routed_scores
        self._info_dicts["top_1_idx_list"] = adapter_ids
        self._cache_routed_adapter_ids(adapter_ids)
        return adapter_ids

    def _route_with_dualprompt_adapter(self, x: torch.Tensor) -> tuple[list[int], list[int]]:
        if self.routing_role == "router_follower":
            expert_ids = self._get_cached_routed_adapter_ids()
            self._info_dicts["top_1_idx_list"] = expert_ids
            return expert_ids, [-1 for _ in expert_ids]
        if not hasattr(self, "dualprompt_router"):
            raise RuntimeError("DualPrompt routing is unavailable for router_follower modules.")
        adapter_input = self._get_adapter_input(x)
        adapter_ids, task_ids, similarities = self.dualprompt_router(adapter_input)
        expert_ids = adapter_ids.tolist()
        task_id_list = task_ids.tolist()
        self._info_dicts["dualprompt_similarities"] = similarities.detach()
        self._info_dicts["dualprompt_task_ids"] = task_id_list
        self._info_dicts["top_1_idx_list"] = expert_ids
        self._cache_routed_adapter_ids(expert_ids)
        return expert_ids, task_id_list

    def compute_l2p_key_loss(self, x: torch.Tensor) -> torch.Tensor:
        if not hasattr(self, "l2p_router"):
            return torch.zeros((), device=x.device, dtype=x.dtype)
        adapter_input = self._get_adapter_input(x)
        _, scores = self.l2p_router(adapter_input, top_k=self.peft_config.l2p_top_k, detach_query=False)
        return -scores.mean()

    def collect_features(self, x: torch.Tensor) -> torch.Tensor:
        return L2PKeyRouter.pool_features(self._get_adapter_input(x))

    def set_dualprompt_general_adapter_id(self, adapter_id: int) -> None:
        if not hasattr(self, "dualprompt_router"):
            return
        self.dualprompt_router.set_general_adapter_id(adapter_id)

    def get_dualprompt_general_adapter_id(self) -> int:
        if not hasattr(self, "dualprompt_router"):
            return -1
        return int(self.dualprompt_router.general_adapter_id.item())

    def set_dualprompt_task_prototype(self, task_id: int, expert_adapter_id: int, prototype: torch.Tensor) -> None:
        if not hasattr(self, "dualprompt_router"):
            return
        self.dualprompt_router.set_task_prototype(task_id, expert_adapter_id, prototype)

    def get_dualprompt_valid_task_count(self) -> int:
        if not hasattr(self, "dualprompt_router"):
            return 0
        return int(self.dualprompt_router.valid_task_mask.sum().item())

    def freeze_adapter(self, adapter_id: int) -> None:
        self._freeze_adapter_module(self.clare_func_adapters[self.adapter_name][adapter_id])
        if self.residual_ensemble_gate_enabled:
            self._freeze_adapter_module(self.clare_residual_ensemble_gates[self.adapter_name][adapter_id])

    def unfreeze_adapter(self, adapter_id: int) -> list[nn.Parameter]:
        params: list[nn.Parameter] = []
        adapter = self.clare_func_adapters[self.adapter_name][adapter_id]
        adapter.train()
        for param in adapter.parameters():
    ...
    def initialize_l2p_pool(self, pool_size: Optional[int] = None) -> list[nn.Parameter]:
        pool_size = int(pool_size or self.peft_config.l2p_pool_size)
        if pool_size <= 0:
            raise ValueError(f"L2P pool_size must be positive, got {pool_size}")
        while self.num_adapters < pool_size:
            self.add_adapter_only(self.num_adapters)
        params: list[nn.Parameter] = []
        for adapter_id in range(self.num_adapters):
            params.extend(self.unfreeze_adapter(adapter_id))
        if hasattr(self, "l2p_router"):
            for param in self.l2p_router.parameters():
                param.requires_grad = True
                params.append(param)
        return params
    ...
    def forward(self, x: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        if self.training:
            ...
            dualprompt_general_adapter_id = -1
            ...
            elif self.routing_mode == "l2p_adapter":
                ignore_forced_l2pdp = os.environ.get("PEFT_L2PDP_IGNORE_FORCED_ADAPTER", "0") == "1"
                if self._forwarded_adapter_id >= 0 and not ignore_forced_l2pdp:
                    adapter_ids = [int(self._forwarded_adapter_id)] * x.shape[0]
                    self._info_dicts["top_1_idx_list"] = adapter_ids
                else:
                    adapter_ids = self._route_with_l2p_adapter(x)
            elif self.routing_mode == "dualprompt_adapter":
                dualprompt_general_adapter_id = self.get_dualprompt_general_adapter_id()
                if dualprompt_general_adapter_id < 0:
                    raise RuntimeError("DualPrompt eval requires a valid general adapter id.")
                ignore_forced_l2pdp = os.environ.get("PEFT_L2PDP_IGNORE_FORCED_ADAPTER", "0") == "1"
                if self._forwarded_adapter_id >= 0 and not ignore_forced_l2pdp:
                    adapter_ids = [int(self._forwarded_adapter_id)] * x.shape[0]
                    self._info_dicts["top_1_idx_list"] = adapter_ids
                else:
                    adapter_ids, _ = self._route_with_dualprompt_adapter(x)
            else:
                if self._forwarded_adapter_id >= 0:
                    adapter_ids = [int(self._forwarded_adapter_id)] * x.shape[0]
                    self._info_dicts["top_1_idx_list"] = adapter_ids
                    self._info_dicts["forced_adapter_id"] = int(self._forwarded_adapter_id)
                else:
            ...
            if os.environ.get("PEFT_DEBUG_L2PDP_ONCE", "0") == "1" and not getattr(self, "_debug_l2pdp_once_printed", False):
                try:
                    print(
                        "[PEFT_DEBUG_L2PDP] "
                        f"layer={self.layer_name} mode={self.routing_mode} role={self.routing_role} "
                        f"forced={self._forwarded_adapter_id} num_adapters={self.num_adapters} "
                        f"adapter_ids={adapter_ids[:8]} general={dualprompt_general_adapter_id}",
                        flush=True,
                    )
                except Exception as exc:
                    print(f"[PEFT_DEBUG_L2PDP_ERROR] {exc}", flush=True)
                self._debug_l2pdp_once_printed = True
            ...
            for idx, _forwarded_adapter_id in enumerate(adapter_ids):
                ...
                if self.use_lora:
                    ...
                    if self.routing_mode == "dualprompt_adapter":
                        adapter_result[idx] = adapter_result[idx] + self._get_eval_adapter_outputs(
                            current_input=current_input,
                            adapter_input=adapter_input,
                            adapter_batch_index=idx,
                            adapter_id=dualprompt_general_adapter_id,
                        )
